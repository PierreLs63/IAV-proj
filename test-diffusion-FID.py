...
def train_diffusion(model, dataloader, diffusion_process, optimizer, device, num_epochs=100, 
    ...
    feature=64,      # permet de garder la résolution 64x64
    normalize=True,  # convertit automatiquement en [0, 255] si float
).to(device)
        
         # Calculer le FID périodiquement avec TorchMetrics
        if (epoch + 1) % fid_eval_freq == 0:
              print(f"\nCalcul du FID (TorchMetrics) à l'epoch {epoch+1}...")
              model.eval()

    # -----------------------------
    # Étape 1 : générer des images
    # -----------------------------
              with torch.no_grad():
    ...
    # -----------------------------------------
    # Étape 2 : Préparation pour TorchMetrics
    # TorchMetrics attend des uint8 en [0,255]
    # -----------------------------------------

              def prepare_for_fid(imgs):
                  # imgs : float32 tensor [0,1] ou [-1,1]
                  if imgs.dtype != torch.uint8:
                      imgs = (imgs.clamp(0,1) * 255).to(torch.uint8)
                  return imgs
              real_images_uint8 = prepare_for_fid(real_images_for_fid)
              generated_uint8   = prepare_for_fid(generated_images)


    # ---------------------------------------------
    # Étape 3 : Calcul du FID (TorchMetrics)
    # ---------------------------------------------
              fid.reset()   # important pour éviter d’accumuler entre epochs

    # Ajouter les vrais (real=True) puis les faux (real=False)
              ...
              print(f"FID TorchMetrics à l'epoch {epoch+1}: {fid_score:.4f}\n")
